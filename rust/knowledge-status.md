...
| 2026-06-17 | 借用冲突规则：不能同时存在 `&T` 和 `&mut T` | planned | 100 | 0 | 自包含示例 |  | 建立读写互斥心智模型；只在 `&mut` 学完后进入 |
| 2026-06-18 | `Option` 的 `Some` / `None` | planned | 97 | 0 | 自包含示例 |  | 已接触但未系统学习；等借用基础稳定后再进入，避免同时引入太多新概念 |
